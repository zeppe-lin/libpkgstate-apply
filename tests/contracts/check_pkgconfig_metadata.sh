...
has_requirement "$public" libpkgapply 4.0.0 || { echo 'pkgconfig-metadata: missing public libpkgapply >= 4.0.0' >&2; exit 1; }
printf '%s\n' "$public" | tr ',' '\n' | awk '$1 == "libpkgapply" && $2 == "<" && $3 == "5.0.0" { found = 1 } END { exit found ? 0 : 1 }' || { echo 'pkgconfig-metadata: missing public libpkgapply < 5.0.0' >&2; exit 1; }
