...
main()
{
  ...
  CHECK(other.projection().lease() == other_lease.identity());
  CHECK(other.projection().snapshot() == projected.projection().snapshot());
  CHECK(other.projection().ownership_inventory() ==
        projected.projection().ownership_inventory());
  return 0;
}
