% LIBPKGSTATE-APPLY(3) libpkgstate-apply | Version 3.1.2
...
Restart-time historical projection is not reconstructed by this boundary. The
durable **libpkgapply** application journal owns the exact projection admitted
before mutation. A caller may use **read_application_state()** under a newly held
lease only to observe current canonical pre-state; that current observation is
never rebased onto an old lease identity.
