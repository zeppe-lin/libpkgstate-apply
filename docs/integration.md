...
completed and before `libpkgstate` publication. It consumes the exact operation
request and completed evidence retained by `libpkgapply`; it does not inspect
mutable build directories, reconstruct incoming authority from planner facts,
acquire target leases, execute effects, or write the state store.
